#!/usr/bin/env bash
# _uninstall-rust.sh — private helper invoked by libs/emu-cua-driver/scripts/uninstall.sh
...
#
# Linux:
#   - ~/.local/bin/emu-cua-driver symlink (only when it resolves to a
#     emu-cua-driver-rs path — a Swift-driver symlink is left in place)
#   - ~/.emu-cua-driver-rs/ (entire package home: telemetry id, install
#     marker, versioned releases, current symlink, lockfile)
#   - ~/.config/systemd/user/emu-cua-driver-rs.service (if --autostart was
#     used via install-local.sh — stop + disable + remove)
#   - Skill symlinks under ~/.claude/skills/emu-cua-driver-rs, ~/.agents/
#     skills/emu-cua-driver-rs, ~/.openclaw/skills/emu-cua-driver-rs,
#     ~/.config/opencode/skills/emu-cua-driver-rs
#
# macOS:
#   - /Applications/EmuCuaDriver.app bundle (and legacy CuaDriverRs.app if present)
#   - ~/.local/bin/emu-cua-driver symlink (only when it resolves into
#     /Applications/EmuCuaDriver.app — see note on the bundle-id share below)
#   - ~/.emu-cua-driver-rs/ (entire package home)
#   - ~/Library/LaunchAgents/com.trycua.emu-cua-driver-rs.plist (if
#     --autostart was used via install-local.sh — unload + remove)
#   - Skill symlinks under ~/.claude/skills/emu-cua-driver-rs, etc.
#
# Also scrubs Claude MCP registrations in ~/.claude.json that point at
# the emu-cua-driver-rs binary.
#
...
# Canonical bundle path (post-rename — shares bundle id `com.emu.cuadriver`
...
LEGACY_APP_BUNDLE="/Applications/EmuCuaDriverRs.app"
HOME_DIR="${CUA_DRIVER_RS_HOME:-$HOME/.emu-cua-driver-rs}"
LAUNCHAGENT_PLIST="$HOME/Library/LaunchAgents/com.trycua.emu-cua-driver-rs.plist"
SYSTEMD_USER_UNIT="$HOME/.config/systemd/user/emu-cua-driver-rs.service"
SKILL_PACK_NAME="emu-cua-driver-rs"
...
# --- CLI symlink ---------------------------------------------------------
#
# Only remove ~/.local/bin/emu-cua-driver when it resolves into a emu-cua-driver-rs
# install. Post-rename, the Rust install lives at /Applications/EmuCuaDriver.app
# — the SAME path the Swift driver uses, with the same bundle id
# `com.emu.cuadriver`. Path-based detection alone can't distinguish
# them. We rely on the presence of `$HOME_DIR` (~/.emu-cua-driver-rs/) — the
# Rust-specific state dir — as the marker that this is a Rust install.
# Pre-rename installs at /Applications/EmuCuaDriverRs.app are still cleaned
# up unambiguously by path.
if [[ -L "$USER_BIN_LINK" ]]; then
    RESOLVED="$(resolve_link "$USER_BIN_LINK")"
    case "$RESOLVED" in
        *"CuaDriverRs.app"*|*"/Applications/EmuCuaDriver.app"*|*"$HOME_DIR"*|*".emu-cua-driver-rs"*)
            rm -f "$USER_BIN_LINK"
            log "removed $USER_BIN_LINK -> $RESOLVED"
            ;;
        *)
            log "$USER_BIN_LINK resolves to $RESOLVED (not a emu-cua-driver-rs path; skipping)"
            ;;
    esac
...
# emu-cua-driver-rs.service. Stop + disable + remove if present so the
...
if [[ "$OS" == "Linux" && -f "$SYSTEMD_USER_UNIT" ]]; then
    if command -v systemctl >/dev/null 2>&1; then
        systemctl --user stop    emu-cua-driver-rs.service 2>/dev/null || true
        systemctl --user disable emu-cua-driver-rs.service 2>/dev/null || true
        log "stopped + disabled systemd --user unit emu-cua-driver-rs.service"
    fi
...
# ~/Library/LaunchAgents/com.trycua.emu-cua-driver-rs.plist. Unload (so the
...
#
# Removes /Applications/EmuCuaDriver.app (current canonical Rust path,
# shared with the Swift driver via bundle id `com.emu.cuadriver`) AND
# /Applications/EmuCuaDriverRs.app (legacy, pre-rename releases).
...
# --- Package home -------------------------------------------------------
#
# Everything under $CUA_DRIVER_RS_HOME (default ~/.emu-cua-driver-rs):
...
# we don't check the target here because `emu-cua-driver skills install`
# writes platform-dependent targets (the local copy under $HOME_DIR/
# skills/emu-cua-driver-rs/), and on Windows the `<HomeDir>/skills` shape
...
#
# Same scrub shape as the Swift uninstall.sh, keyed on the emu-cua-driver-rs
...
def invokes_cua_driver_rs(server):
    ...
    # explicit ".emu-cua-driver-rs" segment. Plain "emu-cua-driver" alone is
    ...
    return ("CuaDriverRs.app" in joined
            or "/Applications/EmuCuaDriver.app" in joined
            or ".emu-cua-driver-rs" in joined
            or "emu-cua-driver-rs" in joined)

def should_remove(name, server):
    return name in {"emu-cua-driver-rs"} or invokes_cua_driver_rs(server)
...
backup = f"{path}.bak-emu-cua-driver-rs-uninstall-{int(time.time())}"
...
PY
    ...
    if [[ -n "$PY_OUTPUT" ]]; then
        ...
        log "no Claude MCP registrations for emu-cua-driver-rs found in $CLAUDE_JSON"
    fi
else
...
if command -v claude >/dev/null 2>&1; then
    for SERVER in emu-cua-driver-rs; do
...
clean re-install flow:
  ...
  (Note: `com.emu.cuadriver` is shared with the Swift emu-cua-driver.
