#!/usr/bin/env bash
# emu-cua-driver uninstaller. Removes everything install.sh laid down — Swift
# driver by default, emu-cua-driver-rs (Rust port) when --experimental-rust /
...
#   - ~/Library/Caches/emu-cua-driver/ (daemon/cache state)
#
...
#
# Flags:
#   --experimental-rust  uninstall the emu-cua-driver-rs (Rust port) backend
#                        instead of the Swift binary. Delegates to
#                        libs/emu-cua-driver/scripts/_uninstall-rust.sh. The
...
# canonical URL works on every platform: `curl … emu-cua-driver/scripts/
...
if [[ "$USE_RUST_BACKEND" == "0" && "$(uname -s 2>/dev/null)" != "Darwin" ]]; then
    USE_RUST_BACKEND=1
    AUTO_RUST=1
    printf 'note: detected non-macOS host (%s); auto-selecting the emu-cua-driver-rs Rust uninstall.\n' \
...
if [[ "$USE_RUST_BACKEND" == "1" ]]; then
    if [[ "$AUTO_RUST" == "0" ]]; then
        ...
        printf 'note: uninstalling emu-cua-driver-rs (Rust port). The Swift binary won'"'"'t be touched.\n' >&2
    else
        ...
        printf 'note: uninstalling emu-cua-driver-rs (the Rust port — canonical on non-macOS).\n' >&2
    fi
    ...
    if [[ -n "$LOCAL_RUST_UNINSTALLER" ]]; then
        ...
        exec /bin/bash -c "$RUST_UNINSTALLER_SCRIPT" emu-cua-driver-rs-uninstall ${FORWARDED_ARGS[@]+"${FORWARDED_ARGS[@]}"}
    fi
fi
...
CACHE_DIR="$HOME/Library/Caches/emu-cua-driver"
...
# that are explicitly named emu-cua-driver or whose command points at a
# emu-cua-driver binary, so unrelated servers named "computer-use" are left
# alone.
...
def invokes_emu_cua_driver(server):
    ...
    joined = " ".join(parts)
    return "emu-cua-driver" in joined or "EmuCuaDriver.app" in joined

def should_remove(name, server):
    return name in {"emu-cua-driver", "emu-cua-computer-use"} or invokes_emu_cua_driver(server)
...
backup = f"{path}.bak-emu-cua-driver-uninstall-{int(time.time())}"
...
PY
    ...
    if [[ -n "$PY_OUTPUT" ]]; then
        ...
        log "no Claude MCP registrations for emu-cua-driver found in $CLAUDE_JSON"
    fi
else
...
if command -v claude >/dev/null 2>&1; then
    for SERVER in emu-cua-driver emu-cua-computer-use; do
