...
enum CLIDocExtractor {
    ...
    private static var callDoc: CommandDoc {
        CommandDoc(
            ...
            discussion: """
                ...
                """,
            arguments: [
                ArgumentDoc(name: "tool-name", help: "Name of the tool to invoke (see `emu-cua-driver list-tools`).", type: "String", isOptional: false),
            ...
            flags: [
                ...
                FlagDoc(name: "no-daemon", shortName: nil, help: "Skip the emu-cua-driver daemon even if one is running.", defaultValue: false),
            ],
            subcommands: []
        )
    }
    ...
    private static var serveDoc: CommandDoc {
        CommandDoc(
            ...
            flags: [
                FlagDoc(name: "no-relaunch", shortName: nil, help: "Stay in the current process instead of re-execing via `open -n -g -a EmuCuaDriver`.", defaultValue: false),
            ],
            subcommands: []
        )
    }
    ...
    private static var mcpConfigDoc: CommandDoc {
        CommandDoc(
            ...
            flags: [
                FlagDoc(name: "claude-code-computer-use-compat", shortName: nil, help: "Print config for Claude Code's window-scoped screenshot compatibility mode registered as `emu-cua-computer-use`.", defaultValue: false),
            ],
            subcommands: []
        )
    }
