...
struct MCPCommand: ParsableCommand {
    ...
    func run() throws {
        ...
        // (shell-spawned bare binary that resolves into a EmuCuaDriver.app
        // bundle), gated by an explicit env / flag opt-out. When the
        // shell already has the right TCC context (e.g. EmuCuaDriver.app
...
extension MCPCommand {
    ...
    fileprivate func shouldUseDaemonProxy() -> Bool {
        ...
        // When AppKit already attributes us to EmuCuaDriver.app — either
        ...
        // EmuCuaDriver.app bundle, otherwise there's nothing for the
    ...
    /// Ensure a `emu-cua-driver serve` daemon is running under the right TCC
    /// context, then run the MCP stdio server with `ListTools` /
    /// `CallTool` handlers that forward every request through
    /// `~/Library/Caches/emu-cua-driver/emu-cua-driver.sock`. Falls back to
    ...
    fileprivate func runViaDaemonProxy() throws {
        ...
        if !DaemonClient.isDaemonListening(socketPath: socketPath) {
            FileHandle.standardError.write(
                Data(
                    "emu-cua-driver: mcp launched without EmuCuaDriver.app's TCC grants; auto-launching the daemon via `open -n -g -a EmuCuaDriver --args serve` and proxying MCP requests through it. Pass --no-daemon-relaunch to stay in-process.\n"
                        .utf8))
            try launchDaemonViaOpen()
            try waitForDaemon(socketPath: socketPath, timeout: 10.0)
        }

        let serverName = claudeCodeComputerUseCompat ? "computer-use" : "emu-cua-driver"
    ...
    /// Spawn `/usr/bin/open -n -g -a EmuCuaDriver --args serve`. Mirror of
    ...
    fileprivate func launchDaemonViaOpen() throws {
        ...
        // -n: force a new instance. EmuCuaDriver.app may already be
        ...
        // -g: keep the new instance backgrounded. EmuCuaDriver.app is
        //     LSUIElement=true anyway, but this makes that explicit.
        process.arguments = ["-n", "-g", "-a", "EmuCuaDriver", "--args", "serve"]
        ...
        do {
            ...
            FileHandle.standardError.write(
                Data(
                    "emu-cua-driver: failed to exec `/usr/bin/open`: \(error). Pass --no-daemon-relaunch to bypass.\n"
                        .utf8))
            throw ExitCode(1)
        }
        ...
        if process.terminationStatus != 0 {
            FileHandle.standardError.write(
                Data(
                    "emu-cua-driver: `open -n -g -a EmuCuaDriver --args serve` exited \(process.terminationStatus). Check that `/Applications/EmuCuaDriver.app` is installed, or pass --no-daemon-relaunch to bypass.\n"
                        .utf8))
            throw ExitCode(1)
        }
    }
    ...
    /// granted Accessibility / Screen Recording to EmuCuaDriver.app yet
    ...
    fileprivate func waitForDaemon(socketPath: String, timeout: TimeInterval) throws {
        ...
        FileHandle.standardError.write(
            Data(
                "emu-cua-driver: daemon did not appear on \(socketPath) within \(Int(timeout))s. If this is the first launch, grant Accessibility + Screen Recording to EmuCuaDriver.app in System Settings and retry. Pass --no-daemon-relaunch to stay in-process.\n"
                    .utf8))
        throw ExitCode(1)
    }
