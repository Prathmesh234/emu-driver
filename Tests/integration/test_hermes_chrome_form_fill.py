...
ONE element in a Chrome window that is NOT frontmost — verifying that:
  ...
  1. emu-cua-driver delivers events to Chromium inputs without stealing focus.
...
1. Kill Chrome, enable "Allow JavaScript from Apple Events" (Preferences
   JSON patch), relaunch Chrome with the form URL via emu-cua-driver's
...
class _HermesChromFormBase(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        # Step 2: Launch Chrome hidden with the form URL via emu-cua-driver's
