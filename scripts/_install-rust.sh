#!/usr/bin/env bash
# _install-rust.sh — private helper invoked by libs/emu-cua-driver/scripts/install.sh
...
#
# Downloads the latest emu-cua-driver-rs release tarball from GitHub Releases
# and drops the binary into ~/.local/bin (or a path given via --bin-dir /
# CUA_DRIVER_RS_INSTALL_DIR). Sudo-free.
#
# This is the Rust port of emu-cua-driver — cross-platform (macOS / Linux /
# Windows via WSL or git-bash) computer-use automation. The Swift
# emu-cua-driver (macOS only) ships separately under tag prefix `emu-cua-driver-v*`
# and is installed via the same `libs/emu-cua-driver/scripts/install.sh` (no
# flag); this helper is hard-pinned to `emu-cua-driver-rs-v*` and will never
...
#                                        (default ~/.emu-cua-driver-rs). Holds
...
#     packages/
#       releases/
#         0.1.3-x86_64-unknown-linux-gnu/emu-cua-driver   (per-version binary)
#         0.1.4-x86_64-unknown-linux-gnu/emu-cua-driver
#       current/emu-cua-driver -> ../releases/<active>/emu-cua-driver  (active version)
#   $CUA_DRIVER_RS_INSTALL_DIR/emu-cua-driver -> $HOME/packages/current/emu-cua-driver
#
...
# and Linux; macOS parity with the Swift emu-cua-driver is in progress. For
# production macOS automation prefer `libs/emu-cua-driver/scripts/install.sh`.
set -euo pipefail

REPO="trycua/cua"
BINARY_NAME="emu-cua-driver"
TAG_PREFIX="emu-cua-driver-rs-v"
...
HOME_DIR="${CUA_DRIVER_RS_HOME:-$HOME/.emu-cua-driver-rs}"
...
# the bare binary so the TCC auto-relaunch path in `emu-cua-driver mcp` has
# a stable bundle id (com.emu.cuadriver) to attribute the daemon to.
# See libs/emu-cua-driver-rs/scripts/EmuCuaDriver.app/Contents/Info.plist and
# the matching docs on `emu-cua-driver mcp`'s auto-relaunch behavior.
# Identical to the Swift driver's EmuCuaDriver.app + com.emu.cuadriver
...
acquire_install_lock() {
    ...
    while ! mkdir "$LOCK_DIR" 2>/dev/null; do
        if (( waited == 0 )); then
            log "another emu-cua-driver-rs install is already in progress (lock at $LOCK_DIR); waiting..."
        fi
...
prune_old_releases() {
    ...
    for ((i=0; i<${#candidates[@]}; i++)); do
        ...
        if (( is_current == 1 )); then
            ...
            # next `emu-cua-driver` invocation).
            continue
        fi
        to_prune+=("$cand")
    done
...
# LABEL  = the release-asset tarball label (matches what cd-rust-emu-cua-driver.yml
...
CUA_DRIVER_RS_BAKED_VERSION="0.2.4"
# ~~~ END_BAKED_VERSION ~~~
...
if [[ -n "${CUA_DRIVER_RS_VERSION:-}" ]]; then
    ...
    # Pinned to the exact `emu-cua-driver-rs-v*` prefix so this script can never
    # accidentally pick up a Swift `emu-cua-driver-v*` release.
    ...
    if [[ -z "$TAG" ]]; then
        ...
        err "  (emu-cua-driver-rs is a BETA-stage cross-platform port; releases may not be published yet.)"
        exit 1
    fi
...
# Tarball selection:
#
# macOS — fetch the directory tarball (emu-cua-driver-rs-vN-darwin-universal.tar.gz).
#   The directory layout includes `EmuCuaDriver.app/` alongside the bare
#   binary, which we need to install into /Applications so the TCC
#   auto-relaunch path in `emu-cua-driver-rs mcp` can resolve
#   `com.emu.cuadriver` via `open -n -g -a CuaDriver`. The
...
case "$LABEL" in
    darwin-*) TARBALL="emu-cua-driver-rs-${VERSION}-darwin-universal.tar.gz" ;;
    *)        TARBALL="emu-cua-driver-rs-${VERSION}-${LABEL}-binary.tar.gz" ;;
esac
...
#     emu-cua-driver-rs-${VERSION}-darwin-universal/
#       ├── emu-cua-driver           (bare universal binary)
#       ├── EmuCuaDriver.app/     (minimal bundle; copy of the same binary
#       │                         lives at Contents/MacOS/emu-cua-driver)
#       └── LICENSE
#   Linux bare-binary tarball expands to:
#     emu-cua-driver               (single file at the archive root)
case "$LABEL" in
    darwin-*)
        STAGE="emu-cua-driver-rs-${VERSION}-darwin-universal"
...
# bin into the bundle so `~/.local/bin/emu-cua-driver` resolves into
# `/Applications/EmuCuaDriver.app/Contents/MacOS/emu-cua-driver`. The
...
# should fire. Same path and same bundle id as the Swift `emu-cua-driver`
# install (`/Applications/EmuCuaDriver.app`, `com.emu.cuadriver`), so an
...
# Linux. Reason: /Applications/EmuCuaDriver.app placement is the
...
# `current` symlink atomically. The visible $BIN_DIR/emu-cua-driver
...
if [[ "$OS" == "Darwin" ]]; then
    if [[ -z "${SRC_APP:-}" || ! -d "$SRC_APP" ]]; then
        ...
        err "  This usually means the downloaded tarball is missing EmuCuaDriver.app — re-run the installer or"
...
if [[ "$OS" == "Darwin" && -n "$SRC_APP" && -d "$SRC_APP" ]]; then
    if [[ ! -w "/Applications" ]]; then
        ...
        err "  Without the .app bundle, \`emu-cua-driver-rs mcp\` from an IDE terminal will not auto-relaunch into a TCC-correct daemon."
        exit 1
    fi
    # The Rust port and the legacy Swift driver both live at
    # /Applications/EmuCuaDriver.app with bundle id `com.emu.cuadriver` —
    ...
    if [[ -e "$APP_DEST" ]]; then
        ...
        if [[ "$PREV_BUNDLE_ID" == "com.emu.cuadriver" ]] && [[ -n "$PREV_BUNDLE_VERSION" ]]; then
            log "replacing existing emu-cua-driver at $APP_DEST (${PREV_BUNDLE_ID}, version ${PREV_BUNDLE_VERSION})"
            REPLACED_SWIFT=1
    ...
    #   releases/<version>-<target>/emu-cua-driver   (this install)
    #   releases/<older>-<target>/emu-cua-driver     (kept for rollback)
    #   current/emu-cua-driver -> ../releases/<active>-<target>/emu-cua-driver
    #
    ...
    # `current/emu-cua-driver` keeps using the open file handle — Unix
    ...
    # + atomic rename instead so a concurrent `emu-cua-driver` lookup
...
# ~/.claude/skills/, ~/.agents/skills/, etc. Run `emu-cua-driver skills
...
# exactly once per install (guarded by ~/.emu-cua-driver-rs/.installation_recorded
# on the binary side). The Rust port keeps its install signal independent
# of the Swift `emu-cua-driver` install (separate marker dir + separate env var)
...
if [[ "$NO_MODIFY_PATH" != "1" ]] && [[ ":$PATH:" != *":$BIN_DIR:"* ]]; then
    ...
    if [[ -n "$SHELL_RC" ]]; then
        {
            printf '\n# Added by emu-cua-driver-rs installer — see https://github.com/trycua/cua\n'
...
echo ""
echo "emu-cua-driver-rs $VERSION installed."
echo ""

if [[ "${REPLACED_SWIFT:-0}" == "1" ]]; then
    echo "Upgraded the emu-cua-driver bundle that was previously at $APP_DEST."
    echo "TCC grants (Accessibility, Screen Recording) are keyed on the bundle id"
    echo "(com.emu.cuadriver) — which is preserved — so they transfer to the new"
...
echo "  $BIN_LINK skills install    # fetch + link the emu-cua-driver-rs skill pack"
