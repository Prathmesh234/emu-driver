...
# the bare binary so the TCC auto-relaunch path in `emu-cua-driver-rs mcp`
# has a stable bundle id (com.emu.cuadriverrs) to attribute the
# daemon to. See libs/emu-cua-driver-rs/scripts/CuaDriverRs.app/Contents/
# Info.plist and the matching docs on `emu-cua-driver-rs mcp`'s auto-
# relaunch behavior. Distinct from the Swift driver's EmuCuaDriver.app
# (com.emu.cuadriver) so the two installs coexist on the same machine.
...
#   `com.emu.cuadriverrs` via `open -n -g -a CuaDriverRs`. The
...
# `/Applications/EmuCuaDriverRs.app/Contents/MacOS/emu-cua-driver`. The
...
# should fire. Same shape as the Swift `emu-cua-driver` install path —
# different bundle id (com.emu.cuadriverrs) so the two coexist.
#
# The macOS path intentionally does NOT use the
# $HOME_DIR/packages/releases/<v>/ + current symlink layout used on
# Linux. Reason: /Applications/EmuCuaDriverRs.app placement is the
