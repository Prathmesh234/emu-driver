...
public enum CuaDriverMCPServer {
    ...
    public static func make(
        serverName: String = "emu-cua-driver",
    ...
    /// every request to a running `emu-cua-driver serve` daemon over its Unix
    ...
    /// correctly attributed to `com.emu.cuadriver` — gives MCP clients
    ...
    private static func fetchProxyToolList(
        ...
        case .noDaemon:
            throw MCPError.internalError(
                "emu-cua-driver daemon not reachable on \(socketPath). "
                    + "Start it with `open -n -g -a EmuCuaDriver --args serve` and retry."
            )
        case .error(let message):
            throw MCPError.internalError(
                "emu-cua-driver daemon transport error while listing tools on \(socketPath): \(message)"
            )
        case .ok(let response):
            guard response.ok, case let .list(listed) = response.result else {
                ...
                throw MCPError.internalError(
                    "emu-cua-driver daemon refused tool list on \(socketPath): \(reason)"
                )
            }
            tools = listed
        }
