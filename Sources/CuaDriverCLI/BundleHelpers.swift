...
/// Shared "is this binary running from inside an installed EmuCuaDriver.app
...
/// `EmuCuaDriver.app/Contents/MacOS/` directory.
///
/// That's the "installed via install-local.sh / install.sh" shape —
/// `~/.local/bin/emu-cua-driver` is a symlink into `/Applications/EmuCuaDriver.app`,
/// and `realpath` walks into the bundle. Returns `false` for `swift run` /
/// raw `.build/<config>/emu-cua-driver` dev invocations, which have no installed
