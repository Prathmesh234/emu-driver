...
/// `emu-cua-driver serve` — binds a Unix domain socket, accepts line-delimited
...
struct ServeCommand: ParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "serve",
        abstract: "Run emu-cua-driver as a long-running daemon on a Unix domain socket.",
        discussion: """
            Listens on --socket (default: ~/Library/Caches/emu-cua-driver/emu-cua-driver.sock).
            Subsequent `emu-cua-driver call/list-tools/describe` invocations auto-detect
            ...
            TCC responsibility chain: when invoked via the `~/.local/bin/emu-cua-driver`
            ...
            EmuCuaDriver.app. AX probes no-op silently and the daemon never becomes
            useful. To sidestep, `serve` detects that context and re-execs itself
            via `open -n -g -a EmuCuaDriver --args serve`, which relaunches under
            LaunchServices so TCC attributes the process to com.emu.cuadriver.
    ...
    @Flag(
        name: .long,
        help: """
            Stay in the current process instead of re-execing via `open -n -g -a EmuCuaDriver`. \
            Use when the calling context already has the right TCC responsibility \
            (running inside EmuCuaDriver.app directly, or from a shell that's been \
    ...
    func run() throws {
        ...
        // socket. Without this check the second `emu-cua-driver serve`
        ...
        AppKitBootstrap.runBlockingAppKitWith {
            ...
            if !granted {
                FileHandle.standardError.write(
                    Data(
                        "emu-cua-driver: required permissions (Accessibility + Screen Recording) not granted; daemon exiting.\n"
                            .utf8))
...
extension ServeCommand {
    ...
    /// via `/usr/bin/open -n -g -a EmuCuaDriver --args serve`. True when all of
    /// the following hold:
    ///
    ...
    ///     `~/.local/bin/emu-cua-driver` symlink from a shell — rather
    ///     than as the main executable of a loaded `.app` bundle. The
    ///     `open -n -g -a` path always lands in the second form
    ///     (bundlePath ends in `/EmuCuaDriver.app`), so checking for its
    ...
    ///     living inside some `EmuCuaDriver.app/Contents/MacOS/`. This
    ///     rules out raw `swift run emu-cua-driver serve` dev invocations,
    ...
    /// com.emu.cuadriver — and the daemon's AppKitBootstrap silently
    /// no-ops. Bouncing through `open` relaunches under LaunchServices so
    /// TCC attributes the fresh process to EmuCuaDriver.app.
    ...
    /// Spawn `/usr/bin/open -n -g -a EmuCuaDriver --args serve [--socket …]`,
    ...
    fileprivate func relaunchViaOpen(socketPath: String) throws {
        FileHandle.standardError.write(
            Data(
                "emu-cua-driver: relaunching via `open -n -g -a EmuCuaDriver --args serve` for correct TCC context. Pass --no-relaunch to stay in this process.\n"
                    .utf8))
        ...
        // -n: force a new instance. Critical — `open -a EmuCuaDriver`
        //     against an already-running EmuCuaDriver process (e.g. a
        //     `emu-cua-driver mcp` started by an MCP client) would re-use
        //     that instance and drop our `--args serve`, leaving us
        //     with "launched something but no serve daemon appeared".
        // -g: keep the new instance in the background. EmuCuaDriver.app is
        //     LSUIElement=true so it wouldn't take focus anyway, but this
        //     makes that explicit.
        process.arguments = ["-n", "-g", "-a", "EmuCuaDriver", "--args", "serve"] + extraArgs
        ...
        do {
            ...
            FileHandle.standardError.write(
                Data(
                    "emu-cua-driver: failed to exec `/usr/bin/open`: \(error)\n"
                        .utf8))
            throw ExitCode(1)
        }
        ...
        if process.terminationStatus != 0 {
            FileHandle.standardError.write(
                Data(
                    "emu-cua-driver: `open -n -g -a EmuCuaDriver --args serve` exited \(process.terminationStatus). Check that `/Applications/EmuCuaDriver.app` is installed, or pass --no-relaunch to bypass.\n"
                        .utf8))
            throw ExitCode(1)
        }
        ...
        if probeReachable {
            FileHandle.standardOutput.write(
                Data(
                    "emu-cua-driver daemon is running (relaunched via EmuCuaDriver.app)\n  socket: \(socketPath)\n"
                        .utf8))
            return
        }
        ...
        FileHandle.standardError.write(
            Data(
                "emu-cua-driver: relaunched EmuCuaDriver.app but no daemon appeared on \(socketPath) within 5s. Check Accessibility + Screen Recording grants for EmuCuaDriver.app, or re-run with --no-relaunch to see in-process errors.\n"
                    .utf8))
        throw ExitCode(1)
    }
    ...
    /// `EmuCuaDriver.app/Contents/MacOS/` directory. That's the "installed
    /// via install-local.sh / install.sh" shape — `~/.local/bin/emu-cua-driver`
    /// is a symlink into `/Applications/EmuCuaDriver.app`, and `realpath`
    ...
    private func resolvedExecutableIsInsideCuaDriverApp() -> Bool {
        ...
        return resolved.contains("/EmuCuaDriver.app/Contents/MacOS/")
    }
...
private func acquireDaemonLockOrExit() throws -> Int32 {
    ...
    if flock(fd, LOCK_EX | LOCK_NB) != 0 {
        ...
        if err == EWOULDBLOCK {
            ...
            FileHandle.standardError.write(
                Data(
                    "emu-cua-driver: flock(\(lockPath)) failed: \(String(cString: strerror(err)))\n"
                        .utf8))
        }
        throw ExitCode(1)
    }
...
/// `emu-cua-driver stop` — sends `{"method":"shutdown"}` to a running daemon
/// and polls for the socket file to disappear. Exits 1 if no daemon was
/// running in the first place.
struct StopCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "stop",
        abstract: "Ask the running emu-cua-driver daemon to exit gracefully."
    )
    ...
    func run() async throws {
        ...
        guard DaemonClient.isDaemonListening(socketPath: socketPath) else {
            FileHandle.standardError.write(
                Data("emu-cua-driver daemon is not running\n".utf8)
            )
            throw ExitCode(1)
        }
        ...
        case .noDaemon:
            FileHandle.standardError.write(
                Data("emu-cua-driver daemon disappeared before shutdown request\n".utf8)
            )
            throw ExitCode(1)
        ...
        FileHandle.standardError.write(
            Data("emu-cua-driver daemon did not release socket within 2s\n".utf8)
        )
        throw ExitCode(1)
    }
}

/// `emu-cua-driver status` — prints whether a daemon is currently reachable
...
struct StatusCommand: AsyncParsableCommand {
    ...
    func run() async throws {
        ...
        // talking to a live emu-cua-driver daemon.
        ...
        case .ok(let response) where response.ok:
            FileHandle.standardOutput.write(
                Data("emu-cua-driver daemon is running\n".utf8)
            )
            FileHandle.standardOutput.write(
