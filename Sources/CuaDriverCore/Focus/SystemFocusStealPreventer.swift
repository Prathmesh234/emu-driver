...
///    leaks surface in `log show --process emu-cua-driver` instead of silently
///    stealing focus.
///
...
private final class Dispatcher: @unchecked Sendable {
    ...
    /// appear in `log show --process emu-cua-driver` and `log stream`. We
    ...
    private let logger = Logger(
        subsystem: "io.trycua.emu-cua-driver", category: "FocusStealPreventer"
    )
