# emu-cua-driver-rs uninstaller (Windows) — removes everything install.ps1
...
#     `emu-cua-driver autostart enable` or install.ps1 -AutoStart)
#   - Any running emu-cua-driver.exe processes (so file handles don't pin
#     the binary directory open during the delete pass)
#   - <visibleBinDir>     = %LOCALAPPDATA%\Programs\trycua\emu-cua-driver-rs\bin  (directory junction)
#   - <currentDir>        = %USERPROFILE%\.emu-cua-driver-rs\packages\current     (directory junction)
#   - <packageHome>       = %USERPROFILE%\.emu-cua-driver-rs\                     (entire tree:
#                                                                              releases, lockfile,
#                                                                              telemetry id,
#                                                                              install marker,
#                                                                              version_check.json)
#   - Skill junctions under:
#       %USERPROFILE%\.claude\skills\emu-cua-driver-rs
#       %USERPROFILE%\.agents\skills\emu-cua-driver-rs
#       %USERPROFILE%\.openclaw\skills\emu-cua-driver-rs
#       %APPDATA%\opencode\skills\emu-cua-driver-rs
...
# `claude mcp remove emu-cua-driver-rs` command for the user to run.
#
...
$AutoStartTask = "emu-cua-driver-serve"

# Skill junctions — mirrors the AGENTS list in
# libs/emu-cua-driver-rs/crates/emu-cua-driver/src/skills.rs (the verb that
# creates them) so we remove from the same paths.
$SkillJunctions = @(
    (Join-Path $env:USERPROFILE ".claude\skills\emu-cua-driver-rs"),
    (Join-Path $env:USERPROFILE ".agents\skills\emu-cua-driver-rs"),
    (Join-Path $env:USERPROFILE ".openclaw\skills\emu-cua-driver-rs"),
    (Join-Path $env:APPDATA      "opencode\skills\emu-cua-driver-rs")
)
...
Write-Step "emu-cua-driver-rs uninstaller (Windows)"
Write-Step "  bin dir     : $VisibleBinDir"
Write-Step "  package home: $HomeDir"
Write-Host ""

# 1. Scheduled Task autostart (registered by `emu-cua-driver autostart enable`
...
# 2. Running emu-cua-driver.exe processes. The serve daemon and any active
#    `emu-cua-driver` invocation hold file handles to the binary, which
#    pin the directory junction's target open and make Remove-Item
#    fail with "in use". Stop them up front so subsequent deletes
#    aren't racy.
$running = @(Get-Process -Name "emu-cua-driver" -ErrorAction SilentlyContinue)
if ($running.Count -gt 0) {
    Write-Step "stopping $($running.Count) running emu-cua-driver.exe process(es)"
    ...
    Write-Step "no running emu-cua-driver.exe processes"
}
...
if (Test-Path -LiteralPath $HomeDir) {
    if (Confirm-Remove "package home tree $HomeDir (releases, lockfile, telemetry id, install marker)") {
        ...
        if (Test-Path -LiteralPath $HomeDir) {
            ...
            Write-WarningStep "  Close any open emu-cua-driver processes / shells with cwd inside the tree and re-run."
        } else {
...
#    in place (a user with a hand-managed emu-cua-driver-rs skill dir
...
Write-Host ""
Write-Host "emu-cua-driver-rs uninstalled." -ForegroundColor Green
Write-Host ""
Write-Host "Claude Code MCP registrations:" -ForegroundColor Yellow
Write-Host "  We don't auto-edit ~/.claude.json on Windows. If you registered emu-cua-driver-rs"
Write-Host "  with Claude Code, remove it manually:"
Write-Host ""
Write-Host "    claude mcp remove emu-cua-driver-rs"
Write-Host ""
Write-Host "  Or edit ~/.claude.json directly and delete entries whose 'command' points at"
Write-Host "  emu-cua-driver.exe under %LOCALAPPDATA%\Programs\trycua\emu-cua-driver-rs\bin\."
Write-Host ""
Write-Host "PATH:"
