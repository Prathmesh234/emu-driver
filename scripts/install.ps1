# emu-cua-driver-rs installer (Windows) — download the latest emu-cua-driver-rs
...
#   irm https://raw.githubusercontent.com/trycua/cua/main/libs/emu-cua-driver/scripts/install.ps1 | iex
#
# Pin a version:
...
#     = %LOCALAPPDATA%\Programs\trycua\emu-cua-driver-rs\bin
#   <currentDir>               [directory junction → release dir]
#     = %USERPROFILE%\.emu-cua-driver-rs\packages\current
#   <release dir>              [real directory, immutable per version]
#     = %USERPROFILE%\.emu-cua-driver-rs\packages\releases\<version>-<target>
#         emu-cua-driver.exe
#
...
#                                    (default %LOCALAPPDATA%\Programs\trycua\emu-cua-driver-rs\bin)
#   $env:CUA_DRIVER_RS_HOME          override the package home
#                                    (default %USERPROFILE%\.emu-cua-driver-rs)
...
#   -AutoStart  register a Scheduled Task that runs `emu-cua-driver serve` at
...
#               `emu-cua-driver --version` works in new shells without manual
...
#
# WARNING — BETA: emu-cua-driver-rs is the cross-platform Rust port of the
# Swift emu-cua-driver. Windows and Linux support is feature-complete;
...
$Repo       = "trycua/cua"
$TagPrefix  = "emu-cua-driver-rs-v"
$BinaryName = "emu-cua-driver.exe"

# Baked-version constant — kept in lock-step with the latest published
# emu-cua-driver-rs-v* release tag by the CD workflow's bake-version step
# (see .github/workflows/cd-rust-emu-cua-driver.yml). The sentinel-block
...
$Script:CuaDriverRsBakedVersion = "0.2.6"
# ~~~ END_BAKED_VERSION ~~~
...
if ($env:CUA_DRIVER_RS_INSTALL_DIR) {
    ...
    $VisibleBinDir = Join-Path $env:LOCALAPPDATA "Programs\trycua\emu-cua-driver-rs\bin"
}

if ($env:CUA_DRIVER_RS_HOME) {
    $HomeDir = $env:CUA_DRIVER_RS_HOME
} else {
    $HomeDir = Join-Path $env:USERPROFILE ".emu-cua-driver-rs"
}
...
function Get-TargetTriple {
    ...
    switch ($osArch) {
        ...
        default {
            ...
            Write-ErrorStep "  emu-cua-driver-rs ships prebuilts for: x86_64 (X64) and arm64 (Arm64)."
...
# Thin wrapper that delegates to `emu-cua-driver autostart enable`. The binary
...
function Register-CuaDriverAutostart {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "emu-cua-driver autostart enable failed (exit $LASTEXITCODE)"
    }
}
...
function Acquire-InstallLock {
    ...
    while ($true) {
        try {
            ...
            if (-not $announced) {
                Write-Step "another emu-cua-driver-rs install is already in progress (lock at $($Script:LockFilePath)); waiting..."
                $announced = $true
            }
    ...
    # $env:USERPROFILE\.emu-cua-driver-rs\install.lock).
    try {
...
function Resolve-Version {
    ...
    # include any emu-cua-driver-rs-v* tag — the repo also ships Swift
    # emu-cua-driver-v* releases plus other unrelated tags, which can push
...
function Get-ReleaseAsset([string]$version, [string]$archLabel, [string]$destDir) {
    $zipName = "emu-cua-driver-rs-$version-$archLabel.zip"
    ...
    # emu-cua-driver-rs-<v>-<arch>\emu-cua-driver.exe (+ LICENSE).
    $stage = "emu-cua-driver-rs-$version-$archLabel"
...
Write-Step "emu-cua-driver-rs installer (Windows)"
...
if (-not $skipDownload) {
    $tmpRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("emu-cua-driver-rs-install-" + [Guid]::NewGuid().ToString("N"))
...
#
# Same shape as the Linux install.sh path: invoke `emu-cua-driver telemetry
# install-event` once per install. The binary itself guards against
# double-counting via ~\.emu-cua-driver-rs\.installation_recorded.
...
#
# We append $VisibleBinDir to the User-scope PATH so `emu-cua-driver` resolves
...
Write-Host ""
Write-Host "emu-cua-driver-rs $version installed."
Write-Host ""
Write-Host "Try it:"
...
if ($onPath) {
    Write-Host "$VisibleBinDir is on your user PATH — emu-cua-driver should resolve in any new shell."
    Write-Host ""
}
elseif ($NoPathUpdate) {
    ...
    try {
        ...
        Write-Host '  Open a new PowerShell window for emu-cua-driver to resolve.'
...
if ($AutoStart) {
    Write-Host ""
    Write-Host "Registering auto-start (emu-cua-driver autostart enable)..." -ForegroundColor Cyan
    try {
        Register-CuaDriverAutostart -InstalledBinary $installedBinary
        Write-Host "  emu-cua-driver serve will auto-start at every interactive logon." -ForegroundColor Green
...
Auto-start at logon (Windows equivalent of macOS LaunchAgent):
  Run emu-cua-driver serve automatically every time you sign in (RDP, console, etc.)
...
Write-Host "Docs: https://github.com/trycua/cua/tree/main/libs/emu-cua-driver-rs"
Write-Host ""
Write-Host "WARNING — BETA: emu-cua-driver-rs is a cross-platform Rust port of the Swift" -ForegroundColor Yellow
Write-Host "          emu-cua-driver. Windows and Linux support is feature-complete; macOS" -ForegroundColor Yellow
