...
1. ``computer_use(action='launch_app', bundle_id='com.apple.Safari',
   ...
   window — verified independently via a direct emu-cua-driver MCP call to
...
Run
---
    python3 -m pytest libs/emu-cua-driver/Tests/integration/test_hermes_launch_safari_bg.py -v
...
def _safari_window_count_via_driver(wait_s: float = 4.0) -> int:
    """Spawn one emu-cua-driver session and poll until Safari windows appear.
...
class TestHermesLaunchSafariBg(unittest.TestCase):
    ...
    def tearDownClass(cls) -> None:
        ...
        subprocess.run(["pkill", "-x", "Safari"], check=False)
        # Kill any orphaned emu-cua-driver processes left by hermes.
        subprocess.run(["pkill", "-f", "emu-cua-driver mcp"], check=False)
    ...
    def test_launch_safari_background_no_focus_steal(self) -> None:
        ...
        # 2. emu-cua-driver must see at least one Safari window.
        ...
        self.assertGreater(
            window_count, 0,
            f"emu-cua-driver sees no Safari windows after launch_app+URL "
            f"(got {window_count}). "
