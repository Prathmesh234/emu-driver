...
/// `emu-cua-driver doctor` — probe TCC / SCK / AX and print a recommendation.
///
...
struct DoctorCommand: AsyncParsableCommand {
    ...
    private func runProbes() async -> DoctorResult {
        ...
        // 2. Attribution check — are we attributed to EmuCuaDriver.app or a shell?
        let bundleID = Bundle.main.bundleIdentifier ?? ""
        let isCorrectBundle = bundleID == "com.emu.cuadriver"
    ...
    private func recommend(
        ...
        if !axOk {
            return Recommendation(
                ...
                detail: """
                    Grant Accessibility to EmuCuaDriver.app in System Settings → Privacy & Security → Accessibility, then restart the daemon:
                      open -n -g -a EmuCuaDriver --args serve
                    If you're running `emu-cua-driver mcp` from a terminal, v0.1.7+ auto-relaunches through EmuCuaDriver.app — make sure you're on the latest release.
                    """
            )
        }
        ...
        if !isCorrectBundle {
            return Recommendation(
                captureMode: nil,
                severity: .warning,
                summary: "TCC is attributed to the wrong process (not EmuCuaDriver.app).",
                detail: """
                    Your shell or IDE is the responsible process for TCC, not EmuCuaDriver.app.
                    Run `emu-cua-driver mcp` v0.1.7+ — it auto-relaunches through EmuCuaDriver.app.
                    Or start the daemon manually: open -n -g -a EmuCuaDriver --args serve
                    """
            )
        }
            ...
            return Recommendation(
                ...
                detail: """
                    ...
                    Workaround: set capture_mode to `ax`:
                      emu-cua-driver config set capture_mode ax
...
struct DoctorResult: Encodable {
    ...
    func formatted() -> String {
        ...
        var lines: [String] = ["── emu-cua-driver doctor ──────────────────────"]
        lines.append("")
        lines.append("System")
