...
#   --experimental-rust  opt into the experimental emu-cua-driver-rs (Rust port)
#                        backend instead of the Swift binary. Delegates to
#                        libs/emu-cua-driver-rs/scripts/install.sh — see that
...
RUST_INSTALLER_URL="https://raw.githubusercontent.com/trycua/cua/main/libs/emu-cua-driver-rs/scripts/install.sh"
...
# path to fall through to — the canonical install becomes emu-cua-driver-rs.
# Auto-set USE_RUST_BACKEND=1 so a single canonical URL works on every
# platform: `curl … emu-cua-driver/scripts/install.sh | bash` gives macOS
...
if [[ "$USE_RUST_BACKEND" == "0" && "$(uname -s 2>/dev/null)" != "Darwin" ]]; then
    USE_RUST_BACKEND=1
    AUTO_RUST=1
    printf 'note: detected non-macOS host (%s); auto-selecting the emu-cua-driver-rs Rust backend.\n' \
...
# rest of argv to emu-cua-driver-rs/scripts/install.sh and exit. The Swift
...
# /Applications/EmuCuaDriverRs.app and re-running this script without the flag.
if [[ "$USE_RUST_BACKEND" == "1" ]]; then
    if [[ "$AUTO_RUST" == "0" ]]; then
        # Explicit opt-in on macOS — flag the "experimental" framing.
        printf 'note: installing experimental Rust backend (emu-cua-driver-rs). The Swift binary won'"'"'t be touched.\n' >&2
    else
        ...
        printf 'note: installing emu-cua-driver-rs (the Rust port — canonical on non-macOS).\n' >&2
    fi
    ...
    if [[ -n "${BASH_SOURCE[0]:-}" && "${BASH_SOURCE[0]}" != "-" && -f "${BASH_SOURCE[0]}" ]]; then
        ...
        CANDIDATE="$SCRIPT_DIR/../../emu-cua-driver-rs/scripts/install.sh"
        if [[ -f "$CANDIDATE" ]]; then
    ...
    if [[ -n "$LOCAL_RUST_INSTALLER" ]]; then
        ...
        exec /bin/bash -c "$RUST_INSTALLER_SCRIPT" emu-cua-driver-rs-install ${FORWARDED_ARGS[@]+"${FORWARDED_ARGS[@]}"}
    fi
fi
...
if [[ -n "${CUA_DRIVER_VERSION:-}" ]]; then
    ...
    # `grep -v emu-cua-driver-rs` is defense-in-depth — the Rust port (emu-cua-driver-rs)
    # publishes to the same repo under tag prefix `emu-cua-driver-rs-v*` and the
    # current grep regex `emu-cua-driver-v[^"]+` already excludes it (differs at
